<template>
  <div>
    <canvas id="opengl-canvas" width="450" height="450"></canvas>
  </div>
</template>

<script>
export default {
  name: 'OpenGL ES Window',
  props: ['vertShader','fragShader','isOpenGLErrored'],
  data: () => {
    return {
      rotate: true,
      timer1: Math.random() * 10000.0,
  ...
  methods: {
    emitError(err) {
      this.error = err;
      this.$emit('onGlError',err);
    },

    startRender() {
      if(this.isOpenGLErrored) {
        return;
      ...
      if(!mat4) {
        this.mitError("mat4 not defined in the browser!");
        return;
      }

      if(this.rotate && this.error == null) {
        requestAnimationFrame(this.startRender);
      }

      const canvas = document.querySelector('#opengl-canvas');
      ...
      const shaderProgram = this.initShaderProgram(gl, this.vertShader, this.fragShader);
    ...
    drawScene(gl, programInfo, buffers) {
      ...
      mat4.rotate(modelViewMatrix,modelViewMatrix, this.timer1 * 1.0, [0.0,0.0,1.0]);
      ...
      // additional uniforms
      gl.uniform1f(programInfo.uniformLocations.seed, this.seed);
      gl.uniform1f(programInfo.uniformLocations.slowTime, this.timer1 * 5.0);
      gl.uniform1f(programInfo.uniformLocations.time, this.timer1 * 2.5);
      ...
      {
        const offset = 0;
        const vertexCount2 = this.vertexCount;
    ...
    initShaderProgram(gl, vsSource, fsSource) {
      const vertexShader = this.loadShader("vertex", gl, gl.VERTEX_SHADER, vsSource);
      const fragmentShader = this.loadShader("fragment", gl, gl.FRAGMENT_SHADER, fsSource);
      ...
      if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) {
        let msg = 'Unable to initialize the shader program: ' + gl.getProgramInfoLog(shaderProgram);
        console.error(msg);
        this.emitError(msg);
    ...
    // compiles it.
    //
    loadShader(name, gl, type, source) {
      ...
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        ...
        let msg = "You have " + name + " shader errors in the console\n\n" + 'An error occurred compiling the ' + name + ' shader:\n' + gl.getShaderInfoLog(shader);
        console.error(msg);
        this.emitError(msg);
  ...
  mounted () {
    ...
    script.setAttribute('src', './assets/js/gl-matrix.js');
    document.head.appendChild(script);

    // inlined gl-matrix script
